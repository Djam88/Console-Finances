...
for (var i = 0; i < finances.length; i++) {
  const record = finances[i];
  const currentDate = record[0];
  const currentProfitLoss = record[1];

  // Calculate total months
  totalMonths++;

  // Calculate total Profit/Losses
  totalProfitLosses += currentProfitLoss;
  ...
  if (i > 0) {
    ...
    // Check for greatest decrease in Profit/Losses
